...
export default function CheckoutPage() {
  ...
  const getStageComponent = () => {
    switch(currentStage) {
      ...
      case 1:
        return <Payment onNext={() => setCurrentStage(2)} onBack={() => setCurrentStage(0)} />;
      case 2:
        return <OrderPlaced />;
      default:
        return null;
        ...
        <CardContent>
          <div className="flex justify-between mb-8 relative">
            {stages.map((stage, index) => (
              <div key={stage} className="flex flex-col items-center w-1/3 z-10">
                <motion.div 
                  ...
                  initial={{ scale: 1 }}
                  animate={{ 
                    scale: completedStages.includes(index) || index === currentStage ? [1, 1.1, 1] : 1,
                  }}
                ...
                <div className={`mt-2 text-sm ${
                  completedStages.includes(index) || index === currentStage ? 'text-gray-700' : 'text-gray-400'
                }`}>
                  {stage}
                </div>
              </div>
